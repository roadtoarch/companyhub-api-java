...
public class CompanyHandler {
  ...
  public ResponseEntity<?> listCompanies(
      Pageable page, @RequestParam Map<String, String> parameters) {
    return ResponseEntity.ok(service.listCompanies(page, parameters));
  }
  ...
  public ResponseEntity<?> createEmployee(@RequestBody CompanyData companyData) {
    ...
    return ResponseEntity.created(
            UriComponentsBuilder.fromPath("/company/{id}")
  ...
  public ResponseEntity<?> updateCompanyPatch(
      @PathVariable Integer companyId, @RequestBody JsonPatch company) {
    try {
      return ResponseEntity.ok(service.updateCompany(companyId, company));
  ...
  public ResponseEntity<?> updateCompany(
      @PathVariable Integer companyId, @RequestBody CompanyData company) {
    try {
      return ResponseEntity.ok(service.updateCompany(companyId, company));
    } catch (CompanyHubException e) {
      return ResponseEntity.status(e.getStatusCode()).body(e.getMessage());
