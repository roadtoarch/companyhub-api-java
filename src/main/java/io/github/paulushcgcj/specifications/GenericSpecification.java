...
public abstract class GenericSpecification<T> {
  ...
  private Predicate buildPredicate(
      CriteriaBuilder criteria, Predicate predicate1, Predicate predicate2) {
  ...
  public Specification<T> filter(Map<String, String> parameters) {
    return (root, query, criteriaBuilder) -> {
      ...
      return queryParams.entrySet().stream()
          ...
          .reduce(
              null,
              (predicate, predicate2) -> buildPredicate(criteriaBuilder, predicate, predicate2));
    };
  }

  private Predicate getPredicate(Root<T> root, CriteriaBuilder criteria, String key, String value) {
    ...
    if (root.get(key).getJavaType() == LocalDateTime.class)
      return criteria.equal(
          root.get(key), LocalDateTime.parse(value, DateTimeFormatter.ISO_LOCAL_DATE_TIME));
    if (root.get(key).getJavaType() == LocalDate.class)
      return criteria.equal(
          root.get(key), LocalDate.parse(value, DateTimeFormatter.ISO_LOCAL_DATE));
    if (root.get(key).getJavaType() == LocalTime.class)
      return criteria.equal(
          root.get(key), LocalTime.parse(value, DateTimeFormatter.ISO_LOCAL_TIME));
